...
if [ "$MERGE_STATUS" -ne 0 ]; then
    ...
    if [ "$UNRESOLVED_FILES" = "$DIST_INDEX" ]; then

        echo ""
        echo "Upstream changed dist/index.js; keeping your local copy..."

        git checkout --ours -- "$DIST_INDEX"
        git add "$DIST_INDEX"

    else

        echo ""
        echo "ERROR: The upstream merge has conflicts that this script cannot resolve safely."
        git status --short
        pause
        exit "$MERGE_STATUS"
...
if git diff --cached --quiet; then
    ...
    git commit -m "Merge upstream/main while preserving manifest.js and dist/index.js"
